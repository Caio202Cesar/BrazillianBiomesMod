...
public class AcaiTreeDecorator extends TreeDecorator {
    public static final Codec<AcaiTreeDecorator> field_236866_a_ = Codec.floatRange(0.0F, 1.0F).fieldOf("probability").xmap(AcaiTreeDecorator::new, (p_236868_0_) -> {
        return p_236868_0_.field_227417_b_;
    }).codec();

    private final float field_227417_b_;

    public AcaiTreeDecorator(float p_i225868_1_) {
        this.field_227417_b_ = p_i225868_1_;
    }

    protected TreeDecoratorType<?> getDecoratorType() {
        return ModTreeDecorators.ACAI.get();
    }
                ...
                for(Direction direction : Direction.Plane.HORIZONTAL) {
                    if (rand.nextFloat() <= 0.25F) {
                        ...
                        if (Feature.isAirAt(world, blockpos)) {
                            BlockState blockstate = TreesGroup.ACAI_BUNCH.get().getDefaultState().with(CocoaBlock.HORIZONTAL_FACING, direction);
