...
public class PurplePassionFruitVineTrunkDecorator extends TreeDecorator {
    public static final Codec<PurplePassionFruitVineTrunkDecorator> CODEC = Codec.unit(PurplePassionFruitVineTrunkDecorator::new);
    public static final PurplePassionFruitVineTrunkDecorator INSTANCE = new PurplePassionFruitVineTrunkDecorator();

    @Override
    protected TreeDecoratorType<?> getDecoratorType() {
        // Return your registered type. Replace with your custom type if desired.
        return ModTreeDecorators.TRUNK_PURPLE_PASSIONVINE_DECORATOR.get();
    }
        ...
        trunkPositions.forEach((pos) -> {
            if (rand.nextInt(3) > 0) {
                ...
                if (Feature.isAirAt(world, blockpos)) {
                    this.placeVine(world, blockpos, PurplePassionfruitVine.EAST, placed, boundingBox);
                }
            }
            if (rand.nextInt(3) > 0) {
                BlockPos blockpos1 = pos.east();
                if (Feature.isAirAt(world, blockpos1)) {
                    this.placeVine(world, blockpos1, PurplePassionfruitVine.WEST, placed, boundingBox);
                }
            }
            if (rand.nextInt(3) > 0) {
                BlockPos blockpos2 = pos.north();
                if (Feature.isAirAt(world, blockpos2)) {
                    this.placeVine(world, blockpos2, PurplePassionfruitVine.SOUTH, placed, boundingBox);
                }
            }
            if (rand.nextInt(3) > 0) {
                BlockPos blockpos3 = pos.south();
                if (Feature.isAirAt(world, blockpos3)) {
                    this.placeVine(world, blockpos3, PurplePassionfruitVine.NORTH, placed, boundingBox);
                }
            }
        });
    }
    ...
    private void placeVine(IWorldGenerationReader world, BlockPos pos, BooleanProperty property, Set<BlockPos> placed, MutableBoundingBox boundingBox) {
        ...
        BlockState vineState = PlantsGroup.PURPLE_PASSION_FRUIT_VINE.get().getDefaultState().with(property, true);
