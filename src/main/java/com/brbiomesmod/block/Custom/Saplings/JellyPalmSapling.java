...
public class JellyPalmSapling extends SaplingBlock {
    public JellyPalmSapling() {
        super(new JellyPalmTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            float minTemp = 0.75f, maxTemp = 1.2f;
    ...
    private static class JellyPalmTree extends Tree {
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
                return TreeFeatures.JELLY_PALM;
        }
    }
}
