...
public class OitiSapling extends SaplingBlock {
    public OitiSapling() {
        super(new OitiTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreesGroup.OITI_SAPLING.get(), RenderType.getCutout());
    ...
    private static class OitiTree extends BigTree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getHugeTreeFeature(Random rand) {
            return TreeFeatures.BIG_OITI_TREE;
        }
        ...
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random randomIn, boolean largeHive) {
            if (randomIn.nextInt(10) == 0) {
                return TreeFeatures.OITI_FANCY_TREE;
            } else {
                return TreeFeatures.OITI_TREE;
            }
        }
    }
}
