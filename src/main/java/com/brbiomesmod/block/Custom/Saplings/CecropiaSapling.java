...
public class CecropiaSapling extends SaplingBlock {
    public CecropiaSapling() {
        super(new CecropiaSapling.CecropiaTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreesGroup.CECROPIA_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreesGroup.POTTED_CECROPIA_SAPLING.get(), RenderType.getCutout());

    }

    //Hardy to zone 10 (in zone 9, it suffer to the same as the fig tree in zone 7 to low, dying back to stump)
    @Override
    ...
    private static class CecropiaTree extends Tree {
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.CECROPIA_TREE;
        }
    }
}
