...
public class GuapuruvuSapling extends SaplingBlock {
    public GuapuruvuSapling() {
        super(new GuapuruvuTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreesGroup.GUAPURUVU_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreesGroup.POTTED_GUAPURUVU_SAPLING.get(), RenderType.getCutout());
    ...
    private static class GuapuruvuTree extends Tree {
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.GUAPURUVU_TREE;
        }
    }
}
