...
public class FicusCestrifoliaSapling extends SaplingBlock {
    public FicusCestrifoliaSapling() {
        super(new FicusCestrifoliaTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreesGroup.FICUS_CESTRIFOLIA_SAPLING.get(), RenderType.getCutout());

    }

    //Hardy from zone 9 to 12
    @Override
    ...
    private static class FicusCestrifoliaTree extends Tree {
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.FICUS_CESTRIFOLIA_TREE;
        }
    }
}
