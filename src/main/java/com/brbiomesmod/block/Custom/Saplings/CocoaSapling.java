...
public class CocoaSapling extends SaplingBlock {
    public CocoaSapling() {
        super(new CocoaTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreesGroup.COCOA_SAPLING.get(), RenderType.getCutout());
    }
    ...
    private static class CocoaTree extends Tree {
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.COCOA_TREE;
        }
    }
}
