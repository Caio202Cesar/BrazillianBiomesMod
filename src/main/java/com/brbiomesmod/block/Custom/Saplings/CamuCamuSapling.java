...
public class CamuCamuSapling extends SaplingBlock {
    public CamuCamuSapling() {
        super(new CamuCamuSapling.CamuCamuTree(), AbstractBlock.Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreesGroup.CAMU_CAMU_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreesGroup.POTTED_CAMU_CAMU_SAPLING.get(), RenderType.getCutout());
    ...
    private static class CamuCamuTree extends Tree {
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.CAMU_CAMU_TREE;
        }
    }
}
