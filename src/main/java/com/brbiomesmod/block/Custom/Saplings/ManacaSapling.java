...
public class ManacaSapling extends SaplingBlock {
    public ManacaSapling() {
        super(new ManacaSapling.ManacaTree(), AbstractBlock.Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreesGroup.MANACA_DA_SERRA_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreesGroup.POTTED_MANACA_DA_SERRA_SAPLING.get(), RenderType.getCutout());

    }

    //Hardy from zone 10 to 11
    @Override
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            float minTemp = 0.85f, maxTemp = 0.94f;
    ...
    private static class ManacaTree extends Tree {
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            if (random.nextInt(10) == 0) {
                return TreeFeatures.MANACA_FANCY_TREE;
            } else {
                return TreeFeatures.MANACA_TREE;
            }
        }
    }
}
