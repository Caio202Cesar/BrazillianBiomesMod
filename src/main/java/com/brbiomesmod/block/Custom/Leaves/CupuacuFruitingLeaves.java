...
public class CupuacuFruitingLeaves extends LeavesBlock implements IForgeShearable {
    private final Supplier<Block> nextStage;

    public CupuacuFruitingLeaves(Properties properties, Supplier<Block> nextStage) {
        super(properties);
    ...
    public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random random) {
        ...
        if ("WINTER".equals(currentSeason) && nextStage != null && random.nextInt(5) == 0) {
            ...
            ItemStack itemStack = new ItemStack(ModItems.CUPUACU.get(), dropCount);
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            worldIn.setBlockState(pos, AmazonRainforestBlocks.CUPUACU_LEAVES.get().getDefaultState());
